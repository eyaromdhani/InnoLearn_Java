...
public class AdminProjectListController implements Initializable {
        ...
        colActions.setCellFactory(column -> new TableCell<Project, Project>() {
            ...
            protected void updateItem(Project project, boolean empty) {
                ...
                if (empty || project == null) {
                    ...
                    viewBtn.setOnAction(e -> handleViewDepots(project));
    ...
    private void handleViewDepots(Project project) {
        try {
            // On utilise MainFX pour charger la page des dépôts
            org.example.MainFX.chargerPage("/DepotList.fxml");

            // Note : Si vous voulez filtrer les dépôts par projet,
            // il faudra passer l'ID du projet au contrôleur de DepotList.
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
