...
import org.example.Services.ServiceGroq;
...
public class CandidatureDetailController {
    ...
    private ServiceGroq serviceGroq = new ServiceGroq();
    ...
    public void setCandidature(StageCondidature sc) {
        ...
        this.serviceOffre = new ServiceOffreStage(MyDataBase.getInstance().getConnection());
        populate(sc);
        calculateAIScore(sc);
    }
    ...
    private void calculateAIScore(StageCondidature sc) {
        ...
        new Thread(() -> {
            try {
                OffreStage offre = serviceOffre.getById(sc.getId_offre());
                if (offre != null) {
