...
public class ServiceQuizResult {
    ...
    public List<QuizResult> getSuspiciousResults(int quizId) throws SQLException {
        List<QuizResult> list = new ArrayList<>();
        String req = "SELECT * FROM quiz_result WHERE formulaire_id = ? AND suspicious_activity IS NOT NULL AND suspicious_activity != 'None' ORDER BY created_at DESC";
        PreparedStatement ps = connection.prepareStatement(req);
        ps.setInt(1, quizId);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
