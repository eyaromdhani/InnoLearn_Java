...
public class PageAdminCoursController {
    ...
    private void chargerCartesCours() {
        ...
        try {
            List<Cours> listeCours = coursService.afficher(); // All courses
    ...
    private VBox createCourseCard(Cours c, List<Categorie_cours> categories) {
        ...
        // Badge niveau
        String niveauRaw = c.getNiveau() != null ? c.getNiveau().toUpperCase() : "INCONNU";
        Label badge = new Label(emojis[index] + " " + niveauRaw);
    ...
    void onAjouterCoursClick(ActionEvent event) {
        try {
            MainFX.chargerPage("/PageAdminAjouterCours.fxml");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    void onModifierCoursClick(Cours cours, ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/PageAdminModifierCours.fxml"));
            Parent root = loader.load();

            PageAdminModifierCoursController controller = loader.getController();
            controller.initData(cours);

            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            stage.setScene(new Scene(root, 1200, 700));
            stage.setTitle("Admin - Modifier un Cours");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
