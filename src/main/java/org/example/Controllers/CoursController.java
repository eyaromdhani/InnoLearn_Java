...
public class CoursController {
    ...
    private VBox creerCard(Cours c, int index) {
        ...
        if (mesFavoris != null && mesFavoris.contains(c.getId())) {
            ...
            coeur.setStyle("-fx-text-fill: #ff3333; -fx-font-size: 26px; -fx-effect: dropshadow(gaussian, rgba(0,0,0,0.4), 4, 0, 0, 2);");
    ...
    @FXML
    public void retour() {
        try { MainFX.chargerPage("/PageEtudiant.fxml"); }
    ...
    public void ouvrirCours() {
        try { MainFX.chargerPageAvecCategorie("/PageCours.fxml", -1); }
        catch (Exception e) { e.printStackTrace(); }
    }
}
