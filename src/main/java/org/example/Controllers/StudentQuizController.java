...
public class StudentQuizController implements Initializable {
    ...
    private void handleGoToAccueil() {
        try {
            // On charge PageEtudiant.fxml au lieu de SelectionView.fxml
            Parent root = FXMLLoader.load(getClass().getResource("/PageEtudiant.fxml"));
            Stage stage = (Stage) quizGrid.getScene().getWindow();
            stage.setTitle("InnoLearn - Espace Étudiant");

            // Astuce : utiliser setRoot évite que la fenêtre ne "saute" ou se redimensionne bizarrement
