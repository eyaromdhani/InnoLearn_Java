...
public class StudentQuestionViewController {
    ...
    private java.util.Map<Question, TextField> answersMap = new java.util.LinkedHashMap<>(); // Preserve order
    ...
    private void handleSubmitQuiz() {
        ...
        for (java.util.Map.Entry<Question, TextField> entry : answersMap.entrySet()) {
        ...
        try {
            QuizResult result = new QuizResult("Etudiant", score, totalPoints, currentQuiz.getId());
    ...
    private void loadResultView(QuizResult result, java.util.List<QuizResultViewController.AnswerDetail> details) {
        try {
    ...
    private void handleBack() {
        if (timeline != null) timeline.stop();
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/StudentQuizDashboard.fxml"));
            Parent root = loader.load();
