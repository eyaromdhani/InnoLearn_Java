...
import org.example.Services.CohereService;
...
public class DepotListController {
    ...
    private void handleBack(javafx.event.ActionEvent event) {
        try {
            // On vérifie si on est dans l'espace Étudiant (présence de contentPane)
            var scene = depotTable.getScene();
            javafx.scene.layout.StackPane contentPane = (javafx.scene.layout.StackPane) scene.lookup("#contentPane");

            if (contentPane != null) {
                // Mode Étudiant : On recharge dans le panneau central
                ...
                // Mode Administrateur : On retourne à la liste d'admin
                org.example.MainFX.chargerPage("/AdminProjectList.fxml");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void handleAddDepot(javafx.event.ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/DepotForm.fxml"));
